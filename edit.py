...
class JsonHandler:
    ...
    def editSection(self, section_id_to_edit: str, new_section_name: str, new_section_id: str):
        if section_id_to_edit in self.data:
            if new_section_id not in self.data:
                self.data[new_section_id]["name"] = new_section_name
                self.data[new_section_id]["id"] = new_section_id
                
                del self.data[section_id_to_edit]
                self.pushJson()
            else:
                raise ValueError(f"Секция с идентификатором {section_id_to_edit} уже существует.")
        else:
                ...
                # управление постами
                    ...
                    while True:
                        ...
                        # Создание поста
                            ...
                            post_title = validated_input("Введите заголовок поста: ")
                            post_id = validated_input("Введите id поста: ")
                            post_explanation = validated_input("Введите обьяснение поста: ")
